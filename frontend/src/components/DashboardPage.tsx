...
const DashboardPage: React.FC = () => {
  ...
  const checkMasterPasswordTestFile = async () => {
    try {
      ...
      console.log(
        "Files being checked for master password test:",
        filesToCheck
      );
  ...
  const handleDeleteFile = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to delete "${selectedItem.split("/").pop()}"?`
      )
    ) {
      return;
    }
    ...
    const checkAuth = async () => {
      try {
        ...
        try {
          ...
          console.log(
            "hasMasterPassword attribute:",
            payload["custom:hasMasterPassword"]
          );
        } catch (e) {
        ...
        if (hasMP && storedPass) {
          ...
          console.log(
            "Master password found in localStorage, proceeding normally"
          );
        } else if (hasMP) {
          // User has set master password but it's not in localStorage
          console.log(
            "Master password exists but not in localStorage, showing enter modal"
          );
  ...
  const handleCreateMasterPassword = async () => {
    ...
    try {
      // *** ADD THIS: Refresh tokens first ***
      // const tokensRefreshed = await refreshUserTokens();
      // if (!tokensRefreshed) {
      //   throw new Error("Failed to refresh authentication tokens.");
      // }
      // 1. Create a test file encrypted with this password
      const testData = new TextEncoder().encode(
        "This is a test file to verify your master password."
      );
      ...
      if (!idToken || !accessToken) {
        throw new Error("Auth toknes are missing after refresh");
      }
  ...
  const handleVerifyMasterPassword = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:8080/verifyMasterPassword",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handlePreview = async () => {
    ...
    const password =
      localStorage.getItem("masterPassword") || "myTestPassword123";
  ...
  const handleLogout = () => {
    ...
    // localStorage.removeItem("masterPassword"); // dont delete this
    navigate("/login");
  };
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download =
        encryptedPackage.fileName.split("/").pop() || "downloaded-file";
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
                ...
                {isPreviewLoading ? (
                  ...
                  selectedItem
                    ...
                    ></iframe>
                      ...
                      .toLowerCase()
                      .match(/\.(txt|md|js|jsx|ts|tsx|html|css|json)$/) ? (
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-gray-500">
                        Click "Download & Decrypt" to access this file
                      </p>
                    </div>
                  )
                ) : (
